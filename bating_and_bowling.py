...
def play(root,flag,wicket_count,bg="#1f1f1f"):
    ...
    canvas = tk.Canvas(root,bg="#1f1f1f", width=1365, height=765)
    canvas.pack()
    frame = Frame(root, width=677, height=300, bg="cyan")
    frame.place(x=0,y=460)
    versus_frame = Frame(root,width=15,height=765,bg="red")
    versus_frame.place(x=(1365//2)-15,y=0)
...
def comp_play(root,flag,wicket_count,bg="white"):
    ...
    #height = root.winfo_screenheight()
    #root.geometry('%dx%d' % (width,height))
    canvas = tk.Canvas(root,width=1365,height=765,bg="white")
    canvas.pack()
